...
namespace boom
{
    ...
    inline APVTS::ParameterLayout createParameterLayout()
    {
        ...
        // Global
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::engine, "Engine", getEngineChoices(), (int)Engine::Drums));
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::timeSig, "Time Signature", getTimeSigChoices(), 0));
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::bars, "Bars", getBarsChoices(), 0));
        ...
        // 808
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::key, "Key", getKeyChoices(), 0));
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::scale, "Scale", getScaleChoices(), 0));
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::octave, "Octave", getOctaveChoices(), 2));
        ...
        // Bass
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::bassStyle, "Bass Style", getStyleChoices(), 0));

        // Drums
        p.emplace_back(std::make_unique<juce::AudioParameterChoice>(ParamIDs::drumStyle, "Drum Style", getStyleChoices(), 0));
