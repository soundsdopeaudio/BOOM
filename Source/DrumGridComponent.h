...
{
public:
    explicit DrumGridComponent(BoomAudioProcessor& p) : processor(p), rowNames(boom::getDefaultDrumRows()) {}
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        for (int bar = 0; bar <= 4; ++bar)
        {
            ...
            if (bar < 4) // label "1.1", "2.1", etc.
            {
                ...
                g.drawText(juce::String(bar + 1) + ".1",
                    x + 4, header.getY() + 2, 48, header.getHeight() - 4,
                    juce::Justification::left);
            }

            // thick bar separator through header + grid
            g.setColour(barLine);
            g.drawLine((float)x, (float)header.getY(),
                (float)x, (float)(header.getBottom() + r.getHeight()),
                2.0f);
        }
        ...
        for (int rr = 0; rr < rows; ++rr)
        {
            g.drawText(rowNames[rr],
                r.getX() + 6, (int)(r.getY() + rr * cellH) + 2,
                juce::jmin(110, r.getWidth() / 6), (int)cellH - 4,
                juce::Justification::left);
        }
        ...
        for (auto& n : pattern)
        {
            ...
            g.fillRoundedRectangle(juce::Rectangle<float>((float)x + 2,
                (float)y + 2,
                cellW - 4,
                cellH - 4),
                4.0f);
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        if (onToggleCell) onToggleCell(rr, c * 24); // 24 ticks per 16th
    }
